...
void i686_PIC_Configure(uint8_t offsetPic1, uint8_t offsetPic2){
    ...
    // Initialization control word 3 (Which one is primary and secondary)
    i686_outb(PIC1_DATA_PORT, 0x4);             // tell PIC1 that it has a secondary at IRQ2 (0000 0100)
    i686_iowait();
...
void i686_PIC_Disable(){
    i686_outb(PIC1_DATA_PORT, 0xFF);        // Full bit mask
    i686_iowait();
    i686_outb(PIC2_DATA_PORT, 0xFF);        // Full bit mask
    i686_iowait();
}
